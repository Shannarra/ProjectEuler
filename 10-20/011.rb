...
(0..1).each do |y|
  (0..1).each do |x|
    ...
    (0..3).each do |n|
      n1 *= peek(x + n, y) # right
      n2 *= peek(x, y + n) # bellow
      n3 *= peek(x + n, y + n) # right diagonal down
      n4 *= peek(x - n, y + n) # left diagonal down
    end
    result = [n1, n2, n3, n4, result].max
  end  
end

assert_and_print result, 16194745